...
| `/price ai` | AI 辅助配置：把价格表 / 账单丢给 agent，让它帮你改（见下） |
| `/price help` | 完整帮助 |
...
改价格不该是填表。开个口子，把价格表、账单、文档原文丢给 agent，它自己读懂后改配置。
...
- **必须你手动启用**。没执行 `/price ai` 时，相关工具对模型完全不可见、调不到，不是靠文案劝住。
- **改动先进草稿**。agent 只能提交结构化改动，不能整份覆盖 JSON；所有引用合法性在落盘前统一校验，引用不存在就拒绝写入。
- **首次落盘前弹确认**，你看过 diff 再决定，同一会话只问一次。
- 附带的 `price-config` skill 会教 agent 工作流（读现状 → 解析资料 → 补缺失信息 → 应用 → 让你确认 → 落盘），信息不全时会来问你，而不是猜一个值填上。
